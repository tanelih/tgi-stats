...
MatchHistoryWorker.prototype.run = function(opts) {
	...
	request(this.endpoint, opts).then(
		...
		function onMatchHistory(data) {
			var matches   = [ ];
			var matchData = data.result.matches;

			for(var i = 0; i < matchData.length; i++) {
				// TODO Figure out a correct format for the 'match' object.
				matches.push({
					'id':      matchData[i].match_id,
					'started': new Date(matchData[i].start_time * 1000)
				});
			}

			// Sort the matches by their 'start_time'.
			matches = _.sortBy(matches, function(m) { return m.started; });
			...
			if(matchData.length > matches.length) return;
			...
			if(data.result.results_remaining) {
				...
				opts = _.merge(opts, {
					'start_at_match_id': _.first(matches).id,
				});
